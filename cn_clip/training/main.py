...
def main():
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if args.distllation:
            num_steps_this_epoch,min_loss = train(model, data, epoch, optimizer, scaler, scheduler, args, steps,min_loss, teacher_model)
        else:
            num_steps_this_epoch,min_loss = train(model, data, epoch, optimizer, scaler, scheduler, args, steps,min_loss)
        ...
        if args.val_data is not None and args.valid_epoch_interval is not None and ((epoch + 1) % args.valid_epoch_interval) == 0:
            ...
            if not args.use_flash_attention:
                lossy = evaluate(model, data, epoch, args, steps)
            else:
