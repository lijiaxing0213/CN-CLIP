...
def train(model, data, epoch, optimizer, scaler, scheduler, args, global_trained_steps,min_loss, teacher_model=None):
    ...
    for i in range(0, dataloader.num_batches):
        ...
        if args.val_data is not None and args.valid_step_interval is not None and ((step + 1) % args.valid_step_interval) == 0:
            ...
            if not args.use_flash_attention:
                lossy = evaluate(model, data, epoch, args, step + 1)
            else:
                # fp16 is needed in flash attention
                with autocast():
                    lossy = evaluate(model, data, epoch, args, step + 1)
            print(lossy,min_loss)
            if lossy < min_loss:
                min_loss = lossy
                save_path = os.path.join(args.checkpoint_path, f"best.pt")
                torch.save(
                    {
                        "epoch": epoch + 1,
                        "step": step,
                ...
                logging.info("Saved best checkpoint {} (epoch {} @ {} steps)".format(save_path, epoch + 1, step))
    ...
    return epoch_trained_steps,min_loss
