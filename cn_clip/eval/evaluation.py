...
def read_submission(submit_path, reference, k=5):
    ...
    with open(submit_path, encoding="utf-8") as fin:
        for line in fin:
            ...
            for rank, image_id in enumerate(item_ids):
                if not isinstance(image_id, int):
                    raise Exception('query_id {} has an invalid predicted image_id {} at rank {}, it should be an integer (not string), please check your schema'.format(qid, image_id, rank + 1))
            # check whether there are duplicate predicted products for a single text
            if len(set(item_ids)) != k:
                raise Exception('query_id {} has duplicate topk images in your prediction. Pleace check again!'.format(qid))
